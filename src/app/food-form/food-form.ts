...
import { Food, FoodFormModel } from '../interface/food-form.interface';
import { v7 } from 'uuid';
...
export class FoodForm {
  ...
  protected mealForm: FormGroup<FoodFormModel> = this.fb.group({
    name: new FormControl('', Validators.required),
    calories: new FormControl({ value: 0, disabled: true }, Validators.required),
  ...
  protected fillFormTest() {
    this.mealForm.patchValue({
      ...
      amount: 1,
      // calories: 240,
      carbohydrates: Math.floor(Math.random() * 80),
  ...
  private createMealFromFormGroup(mealForm: FormGroup<FoodFormModel>): Food {
    return {
      id: v7(),
      name: mealForm.controls.name.value,
  ...
  private openCaloriesCalculationSubscription() {
    const caloriesCalculationSubscription = combineLatest([
        ...
        tap(([amount, protein, fats, carbohydrates]) => {
          if (!!amount && !!protein && !!fats && !!carbohydrates) {
            this.mealForm.controls.calories.setValue(
              this.calculateCaloriesFromMacros(amount, protein, fats, carbohydrates),
            );
          } else {
  ...
  private calculateCaloriesFromMacros(
    ...
    return (fats * 9 + carbohydrates * 4 + protein * 4) * amount;
  }
}
